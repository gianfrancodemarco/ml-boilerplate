...
class CocoAnnotationsManager(AnnotationsManager):
    ...
    def get_segmentations(self):
        ...
        return [annotation["segmentation"] for annotation in annotations]

    def get_flattened_segmentations(self):
        """
        Return flattened segmentations for all images
        All the segmentations points for one image are concatenated into a single list
        
        Segmentations are stored as:
        [ 
            # IMAGE 0
            [
                [
        ...
        annotations = self.get_annotations()
        return [flatten_list(annotation['segmentation']) for annotation in annotations]

    def get_image_segmentations(self, image_id) -> List:
        """
    ...
    def get_segmentation_image_polygons(self, image_id) -> List[Polygon]:
        """
        Given the image id, this method returns the polygon representing the segmentations of the image annotation
        """
        segmentations = self.get_image_segmentations(image_id)
        polygons = []
